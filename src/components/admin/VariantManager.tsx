...
function hydrateFromVariants(variants: VariantDraft[]): ColorRow[] {
...
function buildVariants(colors: ColorRow[]): VariantDraft[] {
  ...
  for (const color of colors) {
    ...
    // Keep color in output even if no sizes selected yet
...
export function VariantManager({ variants, onChange }: VariantManagerProps) {
  const [colors, setColors] = useState<ColorRow[]>(() => hydrateFromVariants(variants));
  ...
  function addSize(colorIndex: number, size: string) {
    ...
    setColors((prev) =>
      prev.map((c, i) => {
        ...
        if (c.sizes.includes(trimmed)) return c;
    ...
    <div>
      ...
      >
        ...
        <input
          ...
          className="flex-1 min-w-[140px] outline-none text-[12px] bg-transparent py-0.5"
        />
      </div>
      {sizes.length === 0 && (
